...
func init() {
	assertSameSize(SFNTName{}, C.FT_SfntName{})
}
...
func (face Face) GetSFNTNameCount() int {
	return int(C.FT_Get_Sfnt_Name_Count(face.face))
}

func (face Face) GetSFNTName(index UInt) (SFNTName, error) {
	var sfntName SFNTName
	err := C.FT_Get_Sfnt_Name(face.face, index, (*C.FT_SfntName)(unsafe.Pointer(&sfntName)))
...
func init() {
	assertSameSize(SFNTLangTag{}, C.FT_SfntLangTag{})
}
...
func (face Face) GetSFNTLangTag(langID UInt) (SFNTLangTag, error) {
	...
	err := C.FT_Get_Sfnt_LangTag(face.face, langID, (*C.FT_SfntLangTag)(unsafe.Pointer(&langTag)))
